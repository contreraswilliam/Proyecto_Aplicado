...
1. **Estadístico de Dickey-Fuller = -8.7951**: Este es el valor calculado para el estadístico de prueba. Un valor más negativo tiende a indicar estacionariedad, ya que refuerza el rechazo de la hipótesis nula de no estacionariedad.
...
# Gráfico ACF
acf_result <- acf(serie_xts, main = "Función de Autocorrelación (ACF)", lag.max = 8)

# Ajustar los límites en el gráfico de autocorrelación
plot(acf_result, ylim = c(-1, 1))
```

```{r echo=FALSE}
# Gráfico PACF
#pacf(serie_xts, main = "Función de Autocorrelacion Parcial (PACF)", lag.max = 8, ylim = c(-1, 1))

pacf_result <- pacf(serie_xts, main = "Función de Autocorrelacion Parcial (PACF)", lag.max = 8)

# Ajustar los límites en el gráfico de autocorrelación
plot(pacf_result, ylim = c(-1, 1))
...
serie_xts <- zoo(Datos_FO$Monto_USD, order.by = fechas)
#serie_xts_fo <- zoo(Datos_FO$Monto_USD, order.by = fechas)

# Asegurarnos de que la frecuencia corresponde al periodo deseado
#serie_xts <- ts(Datos_FP$Monto_USD, frequency = 365, start = c(2023, 1))
 
# Calcula la tendencia usando una media móvil, ajusta el número de periodos según sea necesario
tendencia <- SMA(Datos_FO$Monto_USD, n = 30) # Media móvil de 30 días
 
# Calcula la estacionalidad (residuo de los datos menos la tendencia)
estacionalidad <- Datos_FO$Monto_USD - tendencia
...
estacionalidad <- rep(estacionalidad_media, length.out = length(Datos_FO$Monto_USD))
 
# Calcula el residuo (parte aleatoria)
residuo <- Datos_FO$Monto_USD - tendencia - estacionalidad
 


df_componentes <- data.frame(
  Fecha = fechas,
  Datos = Datos_FO$Monto_USD,
  Tendencia = tendencia,
...
```{r echo=FALSE}
serie_xtsp <- ts(serie_xts, frequency = 7)
modelo_arima <- auto.arima(serie_xtsp, seasonal = TRUE)
...
# Supongamos que tienes tu serie de tiempo 'serie_xts'
# Aplicar cpt.mean para detectar cambios en la media
punto_cambio <- cpt.mean(serie_xts)
...
```{r echo=FALSE}
z_scores <- (serie_xtsp - mean(serie_xtsp, na.rm = TRUE)) / sd(serie_xtsp, na.rm = TRUE)
umbral <- 2
outliers_z <- which(abs(z_scores) > umbral)
outliers_valores <- serie_xtsp[outliers_z]

plot(serie_xtsp, type = "l", main = "Detección de Outliers con Z-score", ylab = "Valor", xlab = "Tiempo")
