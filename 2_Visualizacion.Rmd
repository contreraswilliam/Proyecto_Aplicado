...
archivo_df <- drive_get("Datos_PGREM_CD.csv")
drive_download(archivo_df, path = "Datos_PGREM_CD.csv", overwrite = TRUE)
Datos_df <- read_csv('Datos_PGREM_CD.csv')
...
Datos_filtrados <- Datos_df %>%
  filter(Fecha_Origen >= as.Date("2023-01-01"))
...
```{r echo=FALSE}
Indice.ts<-ts(Datos_FO$Monto_USD, start =c(2023,1), frequency = 365 )
class(Indice.ts)
```
...
Se puede evidenciar que comienza el día 1 del 2023 y finaliza el día 285 del año 2024.
